...
#计算节点意见与其邻居平均意见之间的 Pearson 相关系数（NCI），返回 float
...
#计算加权的全局不一致度，遍历每条边按权重累加意见差平方，最终乘 0.5。
...
def generate_names(n: int, s: int, country_alpha2='US'):
    ...
    使用 names_dataset.NameDataset 从指定国家的热门男女名字中抽取不重复名字，保证偶数男女比例
    '''
...
#向 API 发送 system+user 消息，尝试解析成 response_type（Pydantic model），最多重试 30 次，失败返回 None。gpt-4o-2024-08-06
def get_completion_from_messages_structured(messages, system_messages="You are a helpful assistant.",
                                            model="deepseek-chat", temperature=1, response_type=update_opinion_response):
    success = False
    retry = 0
    max_retries = 30
...
#从 JSON 文件读取所有 agent 的 beliefs 历史，返回按时间步索引的字典
